...
def setup_employee_routes(app, db, Employee, Attendance, Settings, LeaveRequest, PayrollHistory, Announcement, AnnouncementComment, UPLOAD_FOLDER):
    ...
    def check_in():
        ...
        except: return jsonify({"message": "❌ خطأ في تحديد الموقع"}), 400
        photo_data = req_data.get('photo')
        if not photo_data: return jsonify({"message": "❌ لم يتم التقاط صورة"}), 400
        try:
            now = datetime.now(); today_date = now.date()
            if Attendance.query.filter_by(user_id=user_id, date=today_date).first(): return jsonify({"message": "❌ سجلت حضور بالفعل اليوم"}), 400
            st = db.session.get(Settings, 1)
            if not st or st.lat is None or st.lng is None or st.radius is None: return jsonify({"message": "❌ النطاق غير محدد في النظام"}), 400
            if haversine(u_lat, u_lng, float(st.lat), float(st.lng)) > float(st.radius): return jsonify({"message": f"❌ أنت خارج النطاق المسموح"}), 400
            ...
            if face_recognition is not None and emp and emp.face_encoding:
                ...
                if len(current_encodings) == 0: return jsonify({"message": "❌ لم يتم العثور على وجه واضح"}), 400
                match = face_recognition.compare_faces([known_encoding], current_encodings[0], tolerance=0.6)
                if not match[0]: return jsonify({"message": "❌ الوجه غير مطابق"}), 400
            ...
            db.session.commit(); return jsonify({"message": f"✅ تم تسجيل الحضور ({status_msg})"})
        except Exception as e: db.session.rollback(); return jsonify({"message": "❌ خطأ في السيرفر"}), 500

    @app.route('/check_out', methods=['POST'])
    def check_out():
        if not session.get('logged_in'): return jsonify({"message": "سجل دخولك أولاً"}), 401
        user_id, now, today = session.get('user_id'), datetime.now(), datetime.now().date()
        try:
            ...
            if record:
                ...
                record.check_out_time, record.work_hours = now.time().replace(microsecond=0), round((now - start_dt).total_seconds() / 3600, 2)
                db.session.commit(); return jsonify({"message": f"تم الانصراف بنجاح. الساعات: {record.work_hours} ✅"})
            return jsonify({"message": "لا يوجد سجل حضور مفتوح لك اليوم ⚠️"})
        except Exception as e: db.session.rollback(); return jsonify({"message": f"خطأ تقني"}), 500
    ...
    def request_leave():
        ...
        if request.method == 'POST':
            try:
                ...
                db.session.commit()
                if session.get('role') == 'Admin': return '''<script>alert("✅ تم إرسال الطلب بنجاح!"); window.location.href="/dashboard";</script>'''
                else: return '''<script>alert("✅ تم إرسال الطلب بنجاح!"); window.location.href="/emp_dashboard";</script>'''
